...
def compute(filings: list[Filing], today: date | None = None, xml=None) -> Metrics:
    ...
    xml_money = xml.financials if (xml and xml.financials.has_financials) else None

    if not filings and not xml_money:
        return Metrics(
    ...
    # XML first. The SOI extract runs 12-24 months behind, so reading it in
    # preference to a filing that is already public shows figures a year or
    # more staler than the organization's own return -- and staler than what a
    # reader can see for free elsewhere.
    def pick(field: str, metric: str):
        if xml_money is not None:
            value = getattr(xml_money, field, None)
            if value is not None:
                return value
        return latest.get(metric) if latest else None

    revenue = pick("total_revenue", "total_revenue")
    expenses = pick("total_expenses", "total_expenses")
    assets = pick("total_assets", "total_assets")
    liabilities = pick("total_liabilities", "total_liabilities")

    net_assets = pick("net_assets", "net_assets")
    ...
    unrestricted = pick("unrestricted_net_assets", "unrestricted_net_assets")
    if xml_money is not None and xml_money.restricted_net_assets is not None:
        restricted = xml_money.restricted_net_assets
    else:
        restricted_parts = [
            v for v in ((latest.get("temp_restricted_net_assets") if latest else None),
                        (latest.get("perm_restricted_net_assets") if latest else None))
            if v is not None
        ]
    ...
    # One 990 XML carries the prior-year column, so year-over-year change does
    # not need two filings on record.
    prior_revenue = None
    if xml_money is not None and xml_money.prior_revenue is not None:
        prior_revenue = xml_money.prior_revenue
    elif prior:
        prior_revenue = prior.get("total_revenue")
    revenue_change = (
        (revenue - prior_revenue) / abs(prior_revenue)
        if prior_revenue and revenue is not None else None
    )

    officer_share = latest.get("officer_comp_pct") if latest else None
    if officer_share is not None and officer_share > 1:
        officer_share = officer_share / 100.0
    if officer_share is None and latest:
    ...
    support_ratio = xml_money.public_support_ratio if xml_money else None
    basis = "Schedule A public support test" if support_ratio is not None else None
    if support_ratio is None and latest:
        support_ratio = _ratio(latest.get("public_support_170"),
                               latest.get("total_support_170"))
        basis = "170(b)(1)(A)(vi)" if support_ratio is not None else None
    if support_ratio is None and latest:
        support_ratio = _ratio(latest.get("public_support_509"),
                               latest.get("total_support_509"))
        basis = "509(a)(2)" if support_ratio is not None else None

    figures_year = (
        xml.tax_year if (xml_money is not None and xml.tax_year)
        else (latest.fiscal_year if latest else None)
    )
    trend, cagr, span = _trend(filings)

    grants_paid = pick("grants_paid", "grants_paid")
    ...
    return Metrics(
        ...
        contribution_share=_ratio(pick("contributions", "contributions"), revenue),
        ...
        staff_over_100k=latest.get("staff_over_100k") if latest else None,
        latest_fiscal_year=figures_year,
        filing_lag_years=(
            today.year - figures_year + (today.month - 12) / 12.0
            if figures_year else None
        ),
    )
...
def evaluate(filings, m: Metrics, status, today: date | None = None,
             latest_filing_year: int | None = None) -> list[Flag]:
    today = today or date.today()
    flags: list[Flag] = []
    latest = filings[0] if filings else None
    ...
    # published its figures. ProPublica lists filings it holds without
    # extracted data separately; ignoring them makes a current filer read as
    # years delinquent.
    filed_through = max(x for x in (latest_filing_year, fy) if x) if (latest_filing_year or fy) else None
    if filed_through:
        filing_lag = today.year - filed_through + (today.month - 12) / 12.0
        if filing_lag >= THRESHOLDS["stale_filing_years"]:
            push(WATCH, "No return filed recently",
                 f"The most recent return on record covers FY{filed_through}, "
                 f"roughly {filing_lag:.0f} years ago. Three consecutive "
                 f"missed years triggers automatic revocation.", filed_through)
        elif fy and filed_through > fy:
            push(CONTEXT, f"FY{filed_through} return filed, figures not yet published",
                 f"The organization has filed through FY{filed_through}, but "
                 f"extracted financials are available only through FY{fy}. The "
                 f"figures below are FY{fy}. This is the normal IRS processing "
                 f"lag, not a filing problem.", filed_through)
