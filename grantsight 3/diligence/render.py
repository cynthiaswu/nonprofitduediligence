...
table.grid tr.critical td:first-child{box-shadow:inset 3px 0 0 var(--critical)}
...
def portfolio_upload_page(error: str | None = None) -> str:
    body = f"""
    <h1 class="lede">Watch your whole grantee list</h1>
    <p class="sub">Public filings tell you about one organization at a time.
    This reads your list and tells you which of them needs a call this quarter,
    how much of your money is exposed, and where you are the funder holding
    them up.</p>
    ...
    <form method="post" action="/portfolio" enctype="multipart/form-data">
      ...
      <h2>Or paste a list</h2>
      <p class="hint" style="margin-top:0">EINs, names, or a mix — one per line.
      Amounts are read if present.</p>
      ...
      <h2>Or check one organization</h2>
      <div class="search" style="margin-top:0">
        <input type="text" name="single" placeholder="Feeding America, or 36-3673599"
    ...
    <ul class="sources">
      <li><b>A queue, not a report.</b> Who needs a call, what happened, and
          the question to ask them</li>
      <li><b>Your exposure.</b> How many of your dollars sit in flagged
          organizations, and in ones with under three months of reserve</li>
      <li><b>Where you are load-bearing.</b> Your grant as a share of each
          organization's revenue</li>
      <li><b>The portfolio as a shape.</b> Size and reserve distribution,
          benchmarked inside each organization's own sector</li>
      <li>Auto-revocation, late filings, revenue drops, deficits, funder
          concentration and leadership turnover for every grantee</li>
    </ul>
...
def _grid(report) -> str:
    ...
    header = (
        ...
        + "<th>Revenue</th><th>Reserve</th><th>Last FY</th><th>Flags</th></tr>"
    )
    rows = []
    for org in organizations:
        ...
        cells.append(f"<td>{org.latest_fiscal_year or '—'}</td>")
