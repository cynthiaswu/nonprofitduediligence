...
def test_identity_confirms_from_the_press_form_of_the_name():
    ...
    org = {"name": "HARBOR STREET YOUTH COALITION, INC.", "city": "Baltimore",
           "state": "MD", "ein": "52-1693387", "website": None}
...
def test_press_never_changes_severity_via_the_narrative(monkeypatch):
    ...
    with_press = brief_mod.build("52-1693387", search=alarming)
    without = brief_mod.build("52-1693387")
    assert with_press.severity == without.severity
    assert [f.title for f in with_press.flags] == [f.title for f in without.flags]
...
def test_brief_states_what_the_search_actually_did(monkeypatch):
    ...
    result = brief_mod.build("52-1693387", search=lambda q: [])
    html = render.brief_page(result)
    assert "Press mentions" in html
    ...
    # a search happened.
    quiet = brief_mod.build("52-1693387")
...
def test_csv_with_headers():
    data = b"Grantee,EIN,Award\nHarbor Street,52-1693387,$50000\nCedar Creek,84-3311902,$40000\n"
    result = intake_mod.from_csv(data)
    assert result.eins == ["521693387", "843311902"]
...
def test_csv_without_headers_or_consistent_columns():
    ...
    data = (b"521693387,Harbor Street Youth Coalition,2024\n"
            b"Cedar Creek,843311902,\n")
    result = intake_mod.from_csv(data)
    assert result.eins == ["521693387", "843311902"]
...
def test_xlsx_round_trip(tmp_path):
    ...
    sheet.append(["Harbor Street Youth Coalition", "52-1693387", "Youth"])
...
def test_pasted_text():
    result = intake_mod.from_text("52-1693387\n84-3311902\nnot an org\n")
    assert result.eins == ["521693387", "843311902"]
    assert len(result.unmatched) == 1
...
def test_name_extraction_strips_labels_and_amounts():
    assert intake_mod._name_from_line(
        "Harbor Street Youth Coalition   EIN 52-1693387   $75,000"
    ) == "Harbor Street Youth Coalition"
    assert intake_mod._name_from_line("52-1693387") is None
...
def test_mislabelled_extension_is_sniffed(tmp_path):
    ...
    sheet.append(["Harbor Street", "52-1693387"])
...
def test_revoked_grantee_is_critical(monkeypatch):
    ...
    result = portfolio_mod.evaluate_one("52-1693387", today=date(2026, 9, 1))
...
def test_990n_filer_is_context_not_a_problem(monkeypatch):
    ...
    result = portfolio_mod.evaluate_one("30-0112233", today=date(2026, 9, 1))
...
def test_one_bad_ein_does_not_stop_the_sweep(monkeypatch):
    ...
    report = portfolio_mod.evaluate(["521693387", "843311902", "136164271"])
    assert len(calls) == 3
...
SEARCH_INDEX = {
    "harbor street youth coalition": [
        {"ein": 521693387, "name": "HARBOR STREET YOUTH COALITION",
    ...
    "cedar creek literacy project": [
        {"ein": 843311902, "name": "CEDAR CREEK LITERACY PROJECT INC",
...
def test_suffix_stripping_still_distinguishes_genuinely_different_names(monkeypatch):
    ...
    assert match.resolved
    assert match.ein == "843311902"      # not the Conservancy
...
def test_resolve_names_splits_resolved_from_unresolved(monkeypatch):
    ...
    assert resolved == {"521693387": "HARBOR STREET YOUTH COALITION"}
...
def test_award_column_is_read():
    data = (b"Grantee,EIN,Award Amount\n"
            b"Harbor Street,52-1693387,\"$275,000\"\n"
            b"Cedar Creek,84-3311902,90000\n")
    result = intake_mod.from_csv(data)
    assert result.awards == {"521693387": 275000.0, "843311902": 90000.0}
    assert result.total_awarded == 365000.0


def test_award_is_found_without_a_header():
    result = intake_mod.from_csv(b"Harbor Street,52-1693387,$275,000\n")
    assert result.awards["521693387"] == 275000.0


def test_the_ein_is_not_mistaken_for_an_award():
    """"EIN 52-1693387" parsed as an award of $1,693,387 before the EIN was
    stripped from the line first."""
    result = intake_mod.from_text("Harbor Street   EIN 52-1693387   $75,000")
    assert result.rows[0].award == 75000.0
    assert intake_mod.from_text("52-1693387").rows[0].award is None
...
def test_repeat_grants_to_one_organization_are_summed():
    data = b"Name,EIN,Amount\nA,52-1693387,$50,000\nA again,52-1693387,$25,000\n"
    result = intake_mod.from_csv(data)
    assert result.awards["521693387"] == 75000.0


def test_grant_share_of_revenue(monkeypatch):
    _portfolio_stubs(monkeypatch, record=FIXTURE, xml_years=[_xml()])
    result = portfolio_mod.evaluate_one(
        "52-1693387", today=date(2026, 9, 1), award=1_305_000)
...
def test_small_grant_share_is_not_flagged(monkeypatch):
    _portfolio_stubs(monkeypatch, record=FIXTURE, xml_years=[_xml()])
    result = portfolio_mod.evaluate_one(
        "52-1693387", today=date(2026, 9, 1), award=20_000)
...
def test_dollars_at_risk_weights_by_award(monkeypatch):
    ...
    big = portfolio_mod.OrgResult(ein="52-1693387", award=500_000)
    big.signals.append(portfolio_mod.Signal(
        "revoked", portfolio_mod.CRITICAL, "Revoked", "detail"))
    small = portfolio_mod.OrgResult(ein="84-3311902", award=5_000)
...
def test_foundation_reserve_is_not_computed(monkeypatch):
    ...
    result = portfolio_mod.evaluate_one("13-6164271", today=date(2026, 9, 1))
...
def test_report_header_and_ein_disambiguation():
    big = portfolio_mod.OrgResult(ein="52-1693387", name="HOPE HOUSE", revenue=1_000_000)
    other = portfolio_mod.OrgResult(ein="84-3311902", name="HOPE HOUSE", revenue=500_000)
    ...
    assert "52-1693387" in html and "84-3311902" in html
...
def test_new_sf_fixtures_trigger_distinct_signals(monkeypatch):
    ...
    result = portfolio_mod.evaluate_one("94-1556677", today=date(2026, 9, 1))
    ...
    result = portfolio_mod.evaluate_one(
        "94-2088341", today=date(2026, 9, 1), award=115_000)
    kinds = {s.kind for s in result.signals}
