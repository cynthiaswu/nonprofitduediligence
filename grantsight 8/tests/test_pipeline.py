...
def test_xml_index_keeps_current_batch_metadata(tmp_path, monkeypatch):
    """Current IRS indexes carry XML_BATCH_ID; keep it so an object can be
    recovered from the live TEOS ZIPs instead of the retired AWS bucket."""
    from diligence import xml990 as xml_mod
    import csv as _csv

    src = tmp_path / "2026_index.csv"
    with src.open("w", newline="") as f:
        w = _csv.writer(f)
        w.writerow(["RETURN_ID", "EIN", "TAX_PERIOD", "RETURN_TYPE", "OBJECT_ID", "XML_BATCH_ID"])
        w.writerow(["1", "521693387", "202506", "990", "202641079349300209", "2026_TEOS_XML_01A"])

    monkeypatch.setattr(xml_mod, "XML_DB", tmp_path / "xml.sqlite3")
    monkeypatch.setattr(xml_mod, "DATA_DIR", tmp_path)
    report = xml_mod.build_index([str(src)])
    assert report["rows"] == 1
    assert xml_mod.object_ids("52-1693387") == [
        ("202641079349300209", 2025, "2026_TEOS_XML_01A", 2026)
    ]


def test_xml_object_no_longer_defaults_to_retired_aws_bucket():
    from diligence import xml990 as xml_mod
    assert "s3.amazonaws.com/irs-form-990" not in xml_mod.OBJECT_URL
    assert "download-xml?object_id={object_id}" in xml_mod.OBJECT_URL


def test_xml_irs_batch_fallback_extracts_requested_object(tmp_path, monkeypatch):
    from diligence import xml990 as xml_mod
    import zipfile as _zipfile

    raw = (XML_DIR / "sample_990.xml").read_bytes()
    archive = tmp_path / "2026_TEOS_XML_01A.zip"
    with _zipfile.ZipFile(archive, "w") as z:
        z.writestr("202641079349300209_public.xml", raw)

    monkeypatch.setattr(xml_mod, "DATA_DIR", tmp_path)
    monkeypatch.setattr(xml_mod, "OBJECT_URL", "")
    monkeypatch.setattr(xml_mod, "get_bytes", None, raising=False)

    # Patch the imported HTTP function used inside _load_from_irs_batch.
    import diligence.http as http_mod
    monkeypatch.setattr(http_mod, "get_bytes", lambda url, dest, ttl_s: archive)
    parsed = xml_mod._load_from_irs_batch(
        "202641079349300209", "2026_TEOS_XML_01A", 2026
    )
    assert parsed is not None
    assert parsed.tax_year == 2023


def test_healthz_reports_a_live_capability(monkeypatch, tmp_path):
    import sqlite3 as _sqlite3
    from fastapi.testclient import TestClient
...
def test_xml_index_explicit_urls_do_not_replace_current_defaults(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(xml990, "default_index_urls", lambda years=4: ["2026.csv", "2025.csv"])
    captured = []
    monkeypatch.setattr(xml990, "_materialize_index_source", lambda source: captured.append(source) or None)
    try:
        xml990.build_index(["2019.csv", "2020.csv"])
    except RuntimeError:
        pass
    assert captured[:2] == ["2026.csv", "2025.csv"]
    assert captured[2:] == ["2019.csv", "2020.csv"]
