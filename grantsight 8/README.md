...
`GRANTSIGHT_XML_OBJECT_URL` can override the per-object XML endpoint. By
default, GrantSight uses ProPublica's current object download endpoint, then
falls back to the IRS's current monthly TEOS ZIPs using `XML_BATCH_ID` from the
IRS index. The retired pre-2022 AWS `irs-form-990` bucket is no longer used.
With neither available, briefs build without the XML sections and say so in
the gaps list.
