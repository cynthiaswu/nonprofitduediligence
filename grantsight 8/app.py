...
def healthz():
    ...
    xml_years = []
    xml_latest_for_wwf = []
    if xml990.XML_DB.exists():
        try:
            conn = sqlite3.connect(f"file:{xml990.XML_DB}?mode=ro", uri=True)
            try:
                xml_years = [r[0] for r in conn.execute(
                    "SELECT tax_year FROM filings WHERE tax_year IS NOT NULL GROUP BY tax_year ORDER BY tax_year DESC"
                ).fetchall()]
                xml_latest_for_wwf = conn.execute(
                    "SELECT object_id, tax_year, batch_id, processing_year FROM filings "
                    "WHERE ein = '521693387' ORDER BY tax_year DESC, object_id DESC LIMIT 3"
                ).fetchall()
            finally:
                conn.close()
        except sqlite3.Error:
    ...
    capabilities = {
        ...
        "current_financials": {
            "live": bool(xml_rows),
            "detail": f"{xml_rows:,} filings indexed; years through FY{max(xml_years) if xml_years else '—'}" if xml_rows
    ...
    return {
        ...
        "xml_index_years": xml_years[:8],
        "xml_index_wwf_52_1693387": [
            {"object_id": r[0], "tax_year": r[1], "batch_id": r[2], "processing_year": r[3]}
            for r in xml_latest_for_wwf
        ],
    }
