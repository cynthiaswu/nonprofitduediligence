...
def build(ein: str, include_peers: bool = False, include_narrative: bool = False,
    ...
    filed_years = [f.fiscal_year for f in brief.filings if f.fiscal_year]
    filed_years += [
        int(f["tax_prd_yr"]) for f in brief.pdf_filings if f.get("tax_prd_yr")
    ]
    if brief.xml and brief.xml.tax_year:
        filed_years.append(brief.xml.tax_year)
    latest_filing_year = max(filed_years) if filed_years else None
