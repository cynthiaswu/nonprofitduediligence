...
def evaluate_one(ein: str, listed_as: str | None = None,
    ...
    # Every year the organization is known to have filed, from either list.
    filed_years = [f.fiscal_year for f in filings if f.fiscal_year]
    filed_years += [
        int(f["tax_prd_yr"])
        for f in ((record or {}).get("filings_without_data") or [])
        if f.get("tax_prd_yr")
    ]
