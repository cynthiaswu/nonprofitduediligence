...
Two ranged requests instead of a 521 MB download. Handles zip64, which is the
normal case here, not an edge case.

Verified against the live server on 20 Sep 2026:
`2026_TEOS_XML_05A.zip` returns **HTTP 206**, `Accept-Ranges: bytes`,
`Content-Range: bytes 521158438-521158637/521158638`, and a zip64 end record.
Ranged reads work.

### Which column names the archive: XML_BATCH_ID, not SUB_DATE

`SUB_DATE` sounds like a date and is not. In every published index — 2023,
2024, 2025, 2026 — it holds a bare four-digit processing year:

    RETURN_ID,FILING_TYPE,EIN,TAX_PERIOD,SUB_DATE,TAXPAYER_NAME,RETURN_TYPE,DLN,OBJECT_ID,XML_BATCH_ID
    24341909,EFILE,760252367,202509,2026,GALVESTON LITTLE LEAGUE INC,990EZ,93492013018736,202630139349201873,2026_TEOS_XML_01A

No month can be derived from it, so the first version of this fix could not
locate anything and said so rather than guessing — which is how the real
column was found.

**`XML_BATCH_ID` names the archive outright**: `2026_TEOS_XML_05A` →
`{base}/2026/2026_TEOS_XML_05A.zip`. No month arithmetic, no candidate list.
Present from the **2024** index onward; absent in 2023 and earlier, where the
per-object mirrors still serve. `build_index` stores it as `batch_id`.

**An index built before this change cannot locate current-year documents.**
`entrypoint.sh` detects the old schema and rebuilds; it also refreshes
monthly, which it previously never did — a stale index reports organizations
as having stopped filing.

Two latent bugs fixed alongside: column lookups used
`cols.get(A) or cols.get(B)`, which treats column index 0 as missing. Now
`is None`.
...
## Round 3 (20 Sep 2026): three more causes, all measured

Everything below was verified against the live IRS server and against Planned
Parenthood's actual FY2025 return (object `202641349349313769`), not inferred.

### 3. XML_BATCH_ID's letter is wrong when a month is split

The index files that return under `2026_TEOS_XML_05A`. It is not in that
archive. Archives partition a month's filings by object id, in order:

    05A  202601149339300000 .. 202621329349203217   (84,172 members)
    05B  202621329349203222 .. 202641419349301759   (76,704 members)

`202641349349313769` is in **05B**. So trust the year and month in the batch
id, enumerate the suffixes A–E, and take the archive that actually contains
the member. Central directories are cached per archive, so a miss is cheap.

### 4. The IRS uses Deflate64 (method 9)

That member is compression method 9, which neither `zlib` nor the standard
library's `zipfile` can inflate. `inflate64` is now a dependency. Without it
the filing is located correctly and fails at the very last step — the exact
failure shape this module exists to stop repeating.

Decompressed output is checked against the uncompressed size in the central
directory. A short or corrupt buffer raises instead of handing the app a
half-parsed return, because a half-parsed return looks like data.

Verified end to end: 26,825 compressed bytes → 209,339 bytes of XML, giving
`CYTotalRevenueAmt` **410,003,513** and `CYTotalExpensesAmt` **425,979,792** —
matching ProPublica's published FY2025 figures to the dollar.

### 5. `<TaxYr>` is the year the fiscal year BEGINS

PPFA's return for the year ending 30 June 2025 carries `<TaxYr>2024</TaxYr>`
and `<TaxPeriodEndDt>2025-06-30</TaxPeriodEndDt>`. ProPublica, the IRS index
(`TAX_PERIOD` 202506) and every funder call that FY2025. The parser preferred
`<TaxYr>`, so it labelled the newest filing a year early — which is
indistinguishable from a missing filing, and is very likely what produced the
"WWF shows FY2024" report. The period end is now the authority.

### 6. The organization name was the accountants

`PreparerFirmGrp` precedes `Filer` in the ReturnHeader schema, so an unscoped
search for `BusinessNameLine1Txt` returned **KPMG LLP** for this filing. The
name lookup is now scoped to `Filer`.

### Note on the "Last FY" column

`render.py` shows `latest_fiscal_year` (the year of the published figures)
under a heading that reads "Last FY". When figures lag the filing, that column
reads older than the return on file. `latest_filing_year` is the filing-recency
field and is what drives the lateness signals.

---

## Round 4 (20 Sep 2026)

### 7. Form 990-T was being read as the annual return

Meals On Wheels People (93-0584318) filed both a **990-T** and a **990** for
FY2025. The index lists both under the same tax year, so "newest filing" took
whichever sorted first — the 990-T, which is the unrelated-business-income tax
return and carries no program financials. `load_for` got a document with no
revenue, so the brief fell back to the extract's FY2023 figures while still
(correctly) reporting FY2025 as filed. That produced "2025 · figures FY2023".

`indexed_filings` now excludes 990-T from both document selection and filing
recency. A 990-T says nothing about whether the annual return was filed.

### 8. XML_BATCH_ID suffix case is inconsistent

The index writes `2024_TEOS_XML_04a` (lowercase) in places. The case-sensitive
match rejected those outright. `archives_for_batch` is now case-insensitive and
tries the index's own spelling first, then the published uppercase forms.

### 9. Revoked rows had no left padding

`table.grid tr.critical td:first-child` set the inset colour bar but, unlike
the `.flagged` rule, no `padding-left`, and the base `td:first-child` sets it
to 0. Critical rows rendered flush against their own bar with the first
character clipped.
