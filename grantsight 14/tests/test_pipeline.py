...
def test_leadership_turnover_detected(monkeypatch):
    ...
    prior = xml990.parse(
        ...
        .replace("<TaxPeriodEndDt>2023-12-31</TaxPeriodEndDt>",
...
def _xml_for_year(year):
    raw = (XML_DIR / "sample_990.xml").read_text().replace(
        "<TaxYr>2023</TaxYr>", f"<TaxYr>{year}</TaxYr>").replace(
...
def test_xml_raises_but_never_lowers_the_filing_year(monkeypatch):
    ...
    raw = (XML_DIR / "sample_990.xml").read_text().replace(
        "<TaxYr>2023</TaxYr>", "<TaxYr>2025</TaxYr>").replace(
        "<TaxPeriodEndDt>2023-12-31</TaxPeriodEndDt>",
...
def test_batch_id_gives_every_archive_for_its_month():
    """The month in XML_BATCH_ID is reliable; the trailing letter is not.

    Measured: the IRS index files Planned Parenthood's FY2025 return under
    2026_TEOS_XML_05A, and the member lives in 05B. So all suffixes for the
    month must be candidates, in order.
    """
    from diligence import irszip

    urls = irszip.archives_for_batch("2026_TEOS_XML_05A")
    assert urls[0].endswith("/2026/2026_TEOS_XML_05A.zip")
    assert urls[1].endswith("/2026/2026_TEOS_XML_05B.zip")
    assert any(u.endswith("05E.zip") for u in urls)
    # A bare year -- what SUB_DATE actually contains -- names no archive.
    assert irszip.archives_for_batch("2026") == []
    assert irszip.archives_for_batch(None) == []
    assert irszip.archives_for_batch("") == []


def test_deflate64_members_are_readable():
    """The IRS compresses some members with Deflate64 (method 9).

    zlib cannot inflate it and neither can the standard library's zipfile, so
    without inflate64 the filing is located correctly and then fails at the
    very last step. Planned Parenthood's FY2025 return is method 9.
    """
    import inflate64

    from diligence import irszip

    payload = b"<Return><TaxYr>2025</TaxYr></Return>" * 500
    deflater = inflate64.Deflater()
    compressed = deflater.deflate(payload) + deflater.flush()

    calls = []

    def fake_range(client, url, start, end):
        calls.append((start, end))
        if start == 0:  # local header
            return b"PK\x03\x04" + b"\x00" * 22 + (0).to_bytes(2, "little") * 2
        return compressed

    original = irszip._range
    irszip._range = fake_range
    try:
        out = irszip._read_member(None, "u", 0, len(compressed), 9)
        assert out == payload
    finally:
        irszip._range = original
...
def test_fiscal_year_is_the_period_end_not_taxyr(tmp_path):
    """A June-year filer must report the year its fiscal year ENDS.

    Planned Parenthood's return for the year ending 30 June 2025 carries
    <TaxYr>2024</TaxYr>. ProPublica, the IRS index (TAX_PERIOD 202506) and
    every funder call that FY2025. Preferring <TaxYr> labelled the newest
    filing a year early, which looks exactly like a missing filing.
    """
    from diligence import xml990

    path = tmp_path / "june.xml"
    path.write_text(
        '<Return><ReturnHeader>'
        '<TaxPeriodEndDt>2025-06-30</TaxPeriodEndDt>'
        '<TaxYr>2024</TaxYr>'
        '<PreparerFirmGrp><PreparerFirmName>'
        '<BusinessNameLine1Txt>KPMG LLP</BusinessNameLine1Txt>'
        '</PreparerFirmName></PreparerFirmGrp>'
        '<Filer><EIN>131644147</EIN><BusinessName>'
        '<BusinessNameLine1Txt>PLANNED PARENTHOOD FEDERATION OF AMERICA</BusinessNameLine1Txt>'
        '</BusinessName></Filer>'
        '</ReturnHeader><ReturnData><IRS990>'
        '<CYTotalRevenueAmt>410003513</CYTotalRevenueAmt>'
        '</IRS990></ReturnData></Return>'
    )
    filing = xml990.parse(path)
    assert filing.tax_year == 2025, "fiscal year ends June 2025, so FY2025"
    # And the organization is the filer, never the firm that prepared it:
    # PreparerFirmGrp precedes Filer in the schema, so an unscoped lookup
    # returns the accountants.
    assert "PLANNED PARENTHOOD" in (filing.name or "")
    assert "KPMG" not in (filing.name or "")


def test_form_990t_is_never_treated_as_the_annual_return(tmp_path, monkeypatch):
    """990-T is the unrelated-business-income return, not the 990.

    Meals On Wheels People filed both a 990-T and a 990 for FY2025. The 990-T
    sorted first, carried no program financials, and the brief silently fell
    back to the extract's FY2023 figures.
    """
    import sqlite3 as _sqlite3
    ...
    conn = _sqlite3.connect(db)
    conn.execute("CREATE TABLE filings (ein TEXT, tax_year INTEGER, "
                 "object_id TEXT, form_type TEXT, sub_date TEXT, batch_id TEXT)")
    conn.executemany("INSERT INTO filings VALUES (?,?,?,?,?,?)", [
        ("930584318", 2025, "202600439339300410", "990T", "2026", "2026_TEOS_XML_02A"),
        ("930584318", 2025, "202610439349300416", "990", "2026", "2026_TEOS_XML_02A"),
    ])
    conn.commit(); conn.close()
    monkeypatch.setattr(xml_mod, "XML_DB", db)

    rows = xml_mod.indexed_filings("93-0584318")
    assert [r["form_type"] for r in rows] == ["990"]
    assert rows[0]["object_id"] == "202610439349300416"


def test_batch_id_suffix_case_is_tolerated():
    """The index writes some suffixes lowercase; those filings must still resolve."""
    from diligence import irszip

    urls = irszip.archives_for_batch("2024_TEOS_XML_04a")
    assert urls, "a lowercase suffix must not be rejected outright"
    assert urls[0].endswith("/2024/2024_TEOS_XML_04a.zip")   # as the index spells it
    assert any(u.endswith("/2024_TEOS_XML_04A.zip") for u in urls)  # as IRS publishes it
