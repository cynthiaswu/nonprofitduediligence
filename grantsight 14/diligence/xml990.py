...
def indexed_filings(ein: str, limit: int = 10) -> list[dict]:
    ...
    try:
        ...
        # Form 990-T is the unrelated-business-income TAX return, not the
        # annual information return. An organization can file both for the
        # same year, and the index lists them under the same tax year, so
        # "newest filing" picked whichever sorted first. Meals On Wheels
        # People filed both for FY2025; the app took the 990-T, found no
        # program financials in it (there are none to find), and fell back to
        # the extract's FY2023 figures while still reporting FY2025 as filed.
        #
        # It is also excluded from filing recency on purpose: a 990-T tells a
        # funder nothing about whether the annual return was filed.
        rows = conn.execute(
            f"SELECT {cols} FROM filings WHERE ein = ? "
            "AND (form_type IS NULL OR "
            "     UPPER(REPLACE(form_type, '-', '')) NOT LIKE '990T%') "
            "ORDER BY tax_year DESC LIMIT ?",
            (clean_ein(ein), limit),
        ).fetchall()
        return [
