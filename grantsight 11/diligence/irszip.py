...
def month_from_sub_date(sub_date: str | None) -> tuple[int, int] | None:
    """(year, month) from an IRS SUB_DATE. Returns None when unusable.

    The index writes these as M/D/YYYY with a time, but tolerate ISO too
    rather than assume one shape and silently drop the other.
    """
    if not sub_date:
        return None
    text = sub_date.strip()
    ...
    if us:
        return int(us.group(3)), int(us.group(1))
    return None


def candidate_zips(year: int, month: int) -> list[str]:
    """Every zip that could hold a filing submitted in that month."""
    return [
        f"{ZIP_BASE}/{year}/{year}_TEOS_XML_{month:02d}{suffix}.zip"
...
def fetch_object(object_id: str, sub_date: str | None,
                 dest: Path | None = None) -> Path:
    """Pull one filing's XML out of the IRS monthly zip. Raises on failure.

    `sub_date` comes from the IRS index and picks the month. Without it there
    is nothing to search but every zip of every year, which is not a thing to
    do inside a web request.
    """
    when = month_from_sub_date(sub_date)
    if when is None:
        raise ZipSourceError(
            "no submission date for this filing, so the monthly zip that "
            "holds it cannot be identified (rebuild the XML index to capture "
            "SUB_DATE)"
        )
    year, month = when
    ...
    with httpx.Client(timeout=TIMEOUT, follow_redirects=True) as client:
        for url in candidate_zips(year, month):
            try:
            ...
            if found is None:
                problems.append(f"{url}: {member} not in this zip")
                continue
    ...
    raise ZipSourceError("; ".join(problems) or "no candidate zip for that month")
...
def probe(object_id: str, sub_date: str | None) -> dict:
    """What the zip route would do, reported rather than guessed at."""
    when = month_from_sub_date(sub_date)
    report: dict = {"sub_date": sub_date, "month": when}
    if when is None:
        report["usable"] = False
        report["detail"] = "no submission date in the index for this filing"
        return report
    try:
        path = fetch_object(object_id, sub_date)
