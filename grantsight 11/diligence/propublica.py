...
def fetch_organization(ein: str, ttl_s: int | None = None) -> dict[str, Any]:
    """Full record for one EIN: organization object plus all filings.

    The default cache window is long enough that a stale response can look
    exactly like a bug in this codebase, so callers that are diagnosing pass
    ttl_s=0 to force a fresh fetch.
    """
    ein = clean_ein(ein)
    try:
        payload = get_json(
            f"{BASE}/organizations/{int(ein)}.json",
