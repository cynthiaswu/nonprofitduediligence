...
def build_index(index_csvs: list[str] | None = None) -> dict:
    ...
    for source in index_csvs:
        ...
        for stream in _text_members(path):
            ...
            # The submission date is what tells us the return exists and when
            # it arrived -- and which IRS monthly zip holds the document.
            sub_i = cols.get("SUB_DATE") or cols.get("SUBMISSION_DATE")
            ...
            for row in reader:
                ...
                batch.append((ein, year, row[obj_i].strip(), form, sub))
                loaded += 1
                if len(batch) >= 10_000:
                    conn.executemany(
...
def indexed_filings(ein: str, limit: int = 10) -> list[dict]:
    ...
    try:
        ...
        cols = "object_id, tax_year, form_type" + (", sub_date" if has_sub else "")
        rows = conn.execute(
        ...
        return [
            {
                ...
                "tax_year": r[1],
                "form_type": r[2],
                "sub_date": (r[3] if has_sub and len(r) > 3 else None),
            }
            for r in rows
        ]
    except sqlite3.Error:
...
def load_for(ein: str) -> Form990XML | None:
    ...
    for row in indexed_filings(ein, limit=3):
        parsed = _load_object(row["object_id"], row.get("sub_date"))
...
def _load_object(object_id: str, sub_date: str | None = None) -> Form990XML | None:
    ...
    if sub_date:
        from . import irszip
        try:
            path = irszip.fetch_object(object_id, sub_date)
            parsed = parse(path)
            LAST_FETCH_ERROR = None
...
def diagnose(ein: str) -> dict:
    ...
    LAST_FETCH_ERROR = None
    top = rows[0]
    parsed = _load_object(top["object_id"], top.get("sub_date"))
    if parsed is not None:
        report["verdict"] = (
    ...
    from . import irszip
    report["source_probe"] = {
        "irs_monthly_zip": irszip.probe(top["object_id"], top.get("sub_date")),
