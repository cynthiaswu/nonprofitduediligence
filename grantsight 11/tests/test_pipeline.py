...
def test_month_from_sub_date_handles_both_shapes():
    from diligence import irszip

    assert irszip.month_from_sub_date("5/14/2026 10:02:00 AM") == (2026, 5)
    assert irszip.month_from_sub_date("2026-05-14") == (2026, 5)
    assert irszip.month_from_sub_date("") is None
    assert irszip.month_from_sub_date(None) is None
    assert "2026_TEOS_XML_05A.zip" in irszip.candidate_zips(2026, 5)[0]
...
def test_ranged_zip_reader_extracts_one_member(tmp_path, monkeypatch):
    ...
    try:
        path = irszip.fetch_object("202641349349313769", "5/14/2026 10:02:00 AM")
...
def test_zip_reader_says_so_when_ranges_are_refused(tmp_path, monkeypatch):
    ...
    httpx.Client = fake_client
    try:
        report = irszip.probe("202641349349313769", "5/14/2026")
