...
| `GRANTSIGHT_XML_INDEX_URLS` | Optional space-separated IRS index CSVs. If omitted, GrantSight automatically uses the current + 3 prior processing years. |
...
| `GRANTSIGHT_REFRESH_DAYS` | IRS exemption-index rebuild window, default 30 days |
| `GRANTSIGHT_XML_REFRESH_DAYS` | Form 990 XML-index rebuild window, default 30 days |
| `GRANTSIGHT_XML_CHECK_INTERVAL_SECONDS` | How often the running service checks whether the XML index needs refresh, default 6 hours |
...
The entrypoint downloads these indexes and builds `xml_index.sqlite3` automatically on first boot and refreshes it when stale. A background loop checks every six hours by default, so a long-running Railway service does not need a redeploy to pick up new IRS processing years. Include the current processing year (2026 here), because a FY2025
return filed in 2026 belongs to the 2026 processing-year index. Roughly 50 MB
per year.

**The filings themselves.** GrantSight now uses a two-stage per-object path:

1. ProPublica's current `download-xml?object_id=...` endpoint (small, fast,
   and current).
2. If that fails, the `XML_BATCH_ID` in the IRS index identifies the current
   monthly TEOS ZIP under `apps.irs.gov`; GrantSight downloads that batch once,
   extracts the requested XML, and caches it.

The old `s3.amazonaws.com/irs-form-990/...` endpoint is retired and is no
longer the default.
