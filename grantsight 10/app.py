    ...
    if probe is not None:
        return {
            ...
            "detail": f"{xml_rows:,} filings indexed; fetched FY{probe.tax_year} "
                      f"for a sample organization",
            "fix": None,
        }
    return {
        "live": False,
        "detail": f"{xml_rows:,} filings indexed, but no source would serve "
                  f"one: {xml990.LAST_FETCH_ERROR}",
        "fix": "check GRANTSIGHT_XML_OBJECT_URL, or mount a local corpus at "
               "GRANTSIGHT_XML_DIR. Diagnose with: python -m diligence.xml990 "
               "--diagnose <EIN>",
    }
