...
# Per-object sources, tried in order.
#
# The IRS announced on 16 December 2021 that it would stop updating the
# irs-form-990 S3 bucket from 31 December 2021, and AWS now marks that dataset
# deprecated with historic data no longer available. Anything pointing there
# returns 404 for every modern filing -- which is exactly what a default of
# that URL did here: the index resolved the right object id and then every
# fetch failed, silently, leaving briefs on stale extract figures while
# looking like the XML was simply unavailable.
#
# GivingTuesday's public datalake mirrors the IRS corpus per object and is
# maintained. The IRS's own distribution is bulk zips only, with no per-object
# endpoint, so a local corpus is the other supported route.
...
def load_years(ein: str, limit: int = 3) -> list[Form990XML]:
    ...
    ein = clean_ein(ein)
    out: list[Form990XML] = []
    for object_id, _year in object_ids(ein, limit=limit):
        parsed = _load_object(object_id)
        if parsed is not None:
            out.append(parsed)
...
def _load_object(object_id: str) -> Form990XML | None:
...
def diagnose(ein: str) -> dict:
    ...
    found = object_ids(ein, limit=5)
    report["object_ids"] = [{"object_id": o, "tax_year": y} for o, y in found]
    if not found:
        report["verdict"] = (
    ...
    if parsed is not None:
        ...
        report["verdict"] = (
            "The index resolved an object id but no source would serve the "
            "file. This is the failure that looks identical to having no XML "
            "at all. Check the sources listed above."
        )
    return report
