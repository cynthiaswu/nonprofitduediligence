...
| `GRANTSIGHT_NEWS=1` | Press mentions via GDELT (free, no key); off by default |
| `GRANTSIGHT_NEWS_PROVIDER=gnews` + `GNEWS_API_KEY` | Press mentions via GNews instead |
| `GRANTSIGHT_NEWS_PROVIDER=url` + `GRANTSIGHT_NEWS_URL` | Press mentions via a generic search API template |
